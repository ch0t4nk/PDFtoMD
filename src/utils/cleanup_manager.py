...
class CleanupManager:
    ...
    def cleanup_batch_files(self):
        ...
        if temp_batch_dir.exists():
            ...
            try:
                ...
                if remaining_files:
                    self.log(
                        f"   ⚠️  temp_batch directory still contains {len(remaining_files)} items"
                    )
    ...
    def cleanup_workspace(self):
        ...
        if (
            converted_dir.exists()
            and converted_dir.is_relative_to(Path.cwd())
            and any(converted_dir.iterdir())
        ):
            try:
    ...
    def comprehensive_cleanup(self):
        ...
        if self.cleaned_count > 0:
            self.log(
                f"✅ Cleanup completed! Removed {self.cleaned_count} orphaned files."
            )
        else:
...
def main():
    ...
    if len(sys.argv) > 1:
        ...
        if command == "batch":
            ...
            print(
                "Usage: python cleanup_manager.py [batch|workspace|temp|directories|backups|all]"
            )
            return
    else:
        cleanup_manager.comprehensive_cleanup()
