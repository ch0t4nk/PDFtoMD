...
def get_python_files() -> list[Path]:
    ...
    python_files = [
        f for f in python_files if f.name not in ["migrate_to_ssot.py", "config.py"]
    ]
...
def analyze_file(file_path: Path) -> dict[str, list[str]]:
    ...
    try:
        ...
        has_config_import = (
            "from config import" in content or "import config" in content
        )
        ...
        if (
            issues["env_access"] or issues["hardcoded_paths"]
        ) and not has_config_import:
...
def create_migration_plan(
    file_path: Path, issues: dict[str, list[str]]
    ...
    try:
        ...
        for pattern, replacement in replacements.items():
            if re.search(pattern, content):
                migrations.append(("REPLACE", pattern, replacement))
...
def apply_migrations(
    file_path: Path, migrations: list[tuple[str, str, str]], backup: bool = False
) -> bool:
...
def main():
    parser = argparse.ArgumentParser(
        description="Migrate MarkPDFDown code to use SSOT configuration"
    )
    parser.add_argument(
        "--dry-run",
        action="store_true",
    ...
    if not args.dry_run:
        ...
        print("  2. Remove any unused import statements")
        print("  3. Update your .env file with the new template")
        print("  4. Run: python config.py  # to test configuration")
